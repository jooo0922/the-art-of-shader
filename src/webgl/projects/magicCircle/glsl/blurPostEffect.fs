...
uniform sampler2D texture; // minBright 값 이상의 밝기를 갖는 프래그먼트들만 추출해서 넘어온 두 번째 렌더타겟 텍스쳐가 들어올 것임.
...
void main() {
  // 참고로 이 셰이더에서 사용하는 가우시안 블러의 반경값은 2.5로 고정함.
  vec4 color = gaussianBlur(texture, vUv, 2.5, resolution, direction);
  gl_FragColor = color;
}