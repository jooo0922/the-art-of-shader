...
void main() {
  ...
  */
  ...
  float angle = radians(rotate * 360.0) + time * 0.1; // 각도 범위는 초기각도 -360 ~ 360 사이의 값에서 시작하여 시간(time)이 지남에 따라 점점 커질 것임.
  ...
  (now * 2.0 - 1.0) * (1500.0 - (delay * 2.0 - 1.0) * 1000.0), // x좌표값: (-1 ~ 1) * (500 ~ 2500) = 대략 -2500 ~ 2500 사이의 x좌표값이 시간흐름, 랜덤 지속시간에 따라 계산되겠군.
  (now * 2.0 - 1.0) * 1000.0, // y좌표값: -1000 ~ 1000 사이의 y좌표값이 시간흐름, 랜덤 지속시간에 따라 계산되겠군(now)
  sin(radians(time * 50.0 + delay + length(position))) * 15.0 // z좌표값: -15 ~ 15 사이의 z좌표값이 시간흐름, 랜덤지속시간, 로컬원점에서 버텍스까지의 거리에 따라 다르게 계산되겠군.
