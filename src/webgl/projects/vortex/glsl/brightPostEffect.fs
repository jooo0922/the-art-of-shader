...
// 사용자 정의 uniforms
uniform float minBright; // 추출할 밝기의 최소 기준값
uniform sampler2D texture; // 아무런 postEffect 가 적용되지 않은 첫번째 렌더타겟 텍스쳐

// 보간변수
varying vec2 vUv; // 보간되어 넘어온 uv좌표값

void main() {
  vec4 bright = max(vec4(0.0), texture2D(texture, vUv) - vec4(minBright));
  gl_FragColor = bright;
}
