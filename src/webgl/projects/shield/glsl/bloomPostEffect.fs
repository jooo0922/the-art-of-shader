...
// 사용자 정의 uniforms
uniform sampler2D texture1; // 아무런 postEffect 가 적용되지 않은 원본 렌더타겟 텍스쳐
...
void main() {
  ...
  vec4 postEffectColor = texture2D(texture2, vUv) * 8.5; // Bloom 을 제외한 모든 postEffect (minBright 보다 밝은 값 추출, 가우시안 블러) 가 적용된 색상을 일정 비율로 곱해서 적용
