...
export default class Points extends THREE.Points {
  constructor() {
    ...
    for (let i = 0; i < NUM; i++) {
      ...
      const radius = Math.random() * 4 + 1; // 1 ~ 5 사이의 랜덤한 반지름값을 리턴받음.
      positions.setXYZ(
        i,
    ...
    const material = new THREE.RawShaderMaterial({
      uniforms: {
        ...
        pixelRatio: {
          value: window.devicePixelRatio, // css 픽셀 1개 당 물리적 픽셀 개수 비율을 유나폼 변수로 전송함.
        },
        noiseTex: {
          value: null,
        },
      ...
      vertexShader: vs,
      fragmentShader: fs,
      transparent: true, // 셰이더의 알파블렌딩을 활성화함.
      blending: THREE.AdditiveBlending, // 블렌드 모드를 가산블렌딩 모드로 적용함 (겹칠수록 밝아짐)
  ...
  update(time: number) {
    if (this.material instanceof THREE.RawShaderMaterial) {
      ...
      this.rotation.set(0, this.material.uniforms.time.value * 0.2, 0); // Points 객체 전체를 시간이 지날때마다 y축으로 0.2배의 속도로 회전시킴
    }
  }
